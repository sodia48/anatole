...
class EtfDirectoryService:
    ...
    async def _prime_cold_start(self) -> None:
        ...
        async with self._cold_start_lock:
            ...
            self._last_cold_start_attempt = now
            ...
            try:
                ...
                # Une tentative échouée pourra être relancée après 60 s.
                pass

    async def _refresh_all(self) -> None:
        before_count = len(self._quote_cache)
        ...
        try:
            ...
            for batch in _chunks(
                ...
                # Une petite pause évite une rafale de requêtes publiques.
                await asyncio.sleep(0.8)

            now = monotonic()

            if (
                self._quote_cache or
                len(self._quote_cache) > before_count
            ):
                self._last_full_refresh = now
            else:
                # Une collecte entièrement vide après un redémarrage
                # pourra être relancée rapidement.
                self._last_full_refresh = (
                    now
