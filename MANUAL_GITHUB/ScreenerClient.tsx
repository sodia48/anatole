        ...
        if (
          ...
          setError(
            caught instanceof Error
              ? caught.message
              : "Le screener n’a pas pu récupérer les données.",
          );
        }
      } finally {
        if (
  ...
  if (!data && !error) {
    return (
      <section className="panel discovery-loading">
        ...
        <div>
          <h1>
            Préparation du Screener{" "}
            {universe === "composite"
              ? "TSX Composite"
              : "TSX 60"}
          </h1>
          <p>
            Calcul du momentum, du RSI et
    ...
    <div className="discovery-page">
      <header className="panel discovery-hero">
        <div>
          ...
          <p>
            {universe === "composite"
              ? "Classement élargi des sociétés canadiennes selon le momentum, la tendance, le RSI et l’activité du volume."
              : "Vue concentrée des 60 grandes capitalisations canadiennes selon les mêmes critères Anatole."}
          </p>
        </div>
        <div className="discovery-score">
          <Sparkles size={20} />
          <strong>{filtered.length}</strong>
          <span>titres visibles</span>
          <small>
            {data?.live_items ?? 0} données
            publiques ·{" "}
            {data?.fallback_items ?? 0} secours
            {loading
              ? " · actualisation…"
              : ""}
          </small>
