...
function groupRowSpan(
  ...
  const ratio = group.weight / largestWeight;
  return ratio >= 0.62 || group.tiles.length >= 8 ? 2 : 1;
}

function tileColumnSpan(
  tile: NormalizedTile,
  largestWeight: number,
  columns: number,
): number {
  ...
  const maximum = columns >= 12 ? 4 : 3;

  return clamp(Math.round(1 + ratio * (maximum - 1)), 1, maximum);
}
...
function tileLayout(
  ...
  const columns = expanded || tiles.length > 18 ? 12 : 6;
  const spans = new Map<string, number>();

  let occupiedColumns = 0;

  for (const tile of tiles) {
    const span = tileColumnSpan(tile, largestWeight, columns);
    spans.set(tile.ticker, span);
    occupiedColumns += span;
  }

  return {
    columns,
    rows: Math.max(1, Math.ceil(occupiedColumns / columns)),
    spans,
  };
}
    ...
    <section className={`panel ${styles.panel}`}>
      <div className={styles.heading}>
        <div>
          ...
          <h2>S&amp;P/TSX 60</h2>
          <p>
            Vue compacte par secteur · taille selon le poids · couleur selon la variation.
          </p>
        </div>
      ...
      <div className={styles.groups}>
        {visibleGroups.map((group) => {
          ...
          const groupStyle: CSSVariables = {
            "--group-column-span": groupColumnSpan(
              group,
              largestGroupWeight,
            ...
            "--tile-columns": layout.columns,
            "--tile-rows": layout.rows,
          };
              ...
              <div className={styles.tiles}>
                {group.tiles.map((tile) => {
                  const tileStyle: CSSVariables = {
                    "--tile-column-span": layout.spans.get(tile.ticker) ?? 1,
                  ...
                  return (
                    ...
                    >
                      ...
                      <small className={styles.tilePrice}>
