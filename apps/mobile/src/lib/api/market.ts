...
export const marketApi = {
  cockpit: (universe: "tsx60" | "composite" = "tsx60", signal?: AbortSignal) => apiRequest<CockpitSnapshot>(`/api/v1/market/cockpit?universe=${universe}`, { timeoutMs: universe === "composite" ? 95_000 : 35_000, signal }),
  ...
  watchlist: (tickers: string[], signal?: AbortSignal) => apiRequest<WatchlistSnapshot>("/api/v1/market/watchlist", { method: "POST", body: JSON.stringify({ tickers }), timeoutMs: 35_000, signal }),
  news: (language: "fr" | "en", signal?: AbortSignal) => apiRequest<NewsSnapshot>(`/api/v1/discovery/news?lang=${language}`, { timeoutMs: 35_000, signal }),
  stockNews: (ticker: string, company: string, language: "fr" | "en", signal?: AbortSignal) => apiRequest<StockNewsSnapshot>(`/api/v1/stocks/${encodeURIComponent(ticker)}/news?company=${encodeURIComponent(company)}&lang=${language}`, { timeoutMs: 35_000, signal }),
  earnings: (signal?: AbortSignal) => apiRequest<EarningsSnapshot>("/api/v1/discovery/earnings-calendar?universe=composite", { timeoutMs: 40_000, signal }),
  calendar: (language: "fr" | "en", signal?: AbortSignal) => apiRequest<CalendarSnapshot>(`/api/v1/discovery/calendar?lang=${language}`, { timeoutMs: 40_000, signal }),
