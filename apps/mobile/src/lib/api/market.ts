...
export const marketApi = {
  cockpit: (universe: "tsx60" | "composite" | "tsxv" = "tsx60", signal?: AbortSignal) => apiRequest<CockpitSnapshot>(`/api/v1/market/cockpit?universe=${universe}`, { timeoutMs: universe === "tsx60" ? 8_000 : 30_000, signal }),
  ...
  provincialCalendar: (region: ProvinceCode, language: "fr" | "en", signal?: AbortSignal) => apiRequest<ProvincialMacroSnapshot>(`/api/v1/discovery/provincial-calendar?region=${encodeURIComponent(region)}&lang=${language}`, { timeoutMs: 10_000, signal }),
