...
import type { TerminalAlert, TerminalOpportunity, TerminalRadarFilters, TerminalRadarItem, TerminalRadarPreset, TerminalRadarSort, TerminalSector, TerminalSnapshot } from "@/src/lib/api/types";
import { useLocale } from "@/src/lib/i18n";
import { useMobileAccount } from "@/src/providers/MobileAccountProvider";
import { colors, radius, spacing, typography } from "@/src/theme/tokens";
import { alertCopy, opportunityLabel, regimeLabel, riskLabel, sectorStateLabel, uniqueRadarItems, type TerminalFeedMode } from "./model";
...
export function TerminalScreen() {
  ...
  const radar = useMemo<TerminalRadarItem[]>(() => {
    if (!snapshot) return [];
    if (snapshot.radar_items?.length) return snapshot.radar_items;
    return uniqueRadarItems(snapshot).map((item) => ({ ...item, volume: 0, average_volume_20d: 0, sma_20: null, sma_50: null, trend: "N/D", source: "N/D", delayed: true, anomaly_types: [] }));
  }, [snapshot]);
  const sectors = useMemo(() => [...new Set(radar.map((item) => item.sector))].sort(), [radar]);
  const visibleRadar = useMemo(() => filterTerminalRadar(radar, { ...filters, sector: sector === "all" ? filters.sector : sector }, radarSort), [filters, radar, radarSort, sector]);
  ...
  const selectPreset = (preset: TerminalRadarPreset) => { setActivePreset(preset.id); setPresetName(preset.name); setFilters(preset.filters); setRadarSort(preset.sort); setSector("all"); };
...
const styles = StyleSheet.create({
  ...
  customFilters: { flexDirection: "row", flexWrap: "wrap", gap: spacing.xs }, input: { minWidth: "46%", flexGrow: 1, minHeight: 44, paddingHorizontal: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: radius.sm, backgroundColor: colors.surfaceRaised, color: colors.text },
