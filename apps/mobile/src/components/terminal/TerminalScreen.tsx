...
export function advancedRadarItems(snapshot: TerminalSnapshot): TerminalRadarItem[] | null {
  const items = (snapshot as Partial<TerminalSnapshot>).radar_items;
...
export function TerminalScreen() {
  ...
  const radar = useMemo(() => snapshot ? advancedRadarItems(snapshot) : null, [snapshot]);
  const legacyRadar = useMemo(() => snapshot ? uniqueRadarItems(snapshot) : [], [snapshot]);
  const radarSource = radar ?? legacyRadar;
  const sectors = useMemo(() => [...new Set(radarSource.map((item) => item.sector))].sort(), [radarSource]);
  const visibleRadar = useMemo(() => radar
    ? filterTerminalRadar(radar, filters, radarSort)
    : filterAndSortRadar(legacyRadar, mode, filters.sector ?? "all"), [filters, legacyRadar, mode, radar, radarSort]);
...
const styles = StyleSheet.create({
  ...
  filterLabel: { ...typography.label, color: colors.primary, textTransform: "uppercase" }, filters: { flexDirection: "row", flexWrap: "wrap", gap: spacing.xs }, filter: { minHeight: 44, justifyContent: "center", paddingHorizontal: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: radius.pill, backgroundColor: colors.surfaceRaised }, filterActive: { borderColor: colors.primary, backgroundColor: "rgba(44,156,255,.18)" }, filterText: { ...typography.caption, color: colors.text, fontWeight: "700" }, legacyRadarNotice: { ...typography.caption, color: colors.warning, padding: spacing.sm, borderWidth: 1, borderColor: colors.warning, borderRadius: radius.sm },
