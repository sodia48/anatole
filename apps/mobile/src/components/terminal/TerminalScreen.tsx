...
export function TerminalScreen() {
  ...
  const [highlightSymbol, setHighlightSymbol] = useState(() => (Array.isArray(params.symbol) ? params.symbol[0] : params.symbol)?.trim().toUpperCase() ?? "");
  const [highlightAnomaly, setHighlightAnomaly] = useState(() => (Array.isArray(params.anomaly) ? params.anomaly[0] : params.anomaly)?.trim() ?? "");
  ...
  const visibleRadar = useMemo(() => filterTerminalRadar(radar, filters, radarSort).filter((item) => (!highlightSymbol || item.symbol.toUpperCase() === highlightSymbol) && (!highlightAnomaly || item.anomaly_types.some((type) => type === highlightAnomaly))), [filters, highlightAnomaly, highlightSymbol, radar, radarSort]);
  ...
  const header = <View style={styles.header}>
    ...
    {snapshot && (highlightSymbol || highlightAnomaly) ? <View style={styles.deepLink} testID="terminal-deep-link"><Text style={styles.deepLinkText}>{[highlightSymbol, highlightAnomaly].filter(Boolean).join(" · ")}</Text><Pressable accessibilityRole="button" onPress={() => { setHighlightSymbol(""); setHighlightAnomaly(""); }} style={styles.deepLinkClear}><Text style={styles.deepLinkText}>{pick("Voir tout", "View all")}</Text></Pressable></View> : null}
...
const styles = StyleSheet.create({
  ...
  deepLink: { minHeight: 44, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: spacing.sm, paddingHorizontal: spacing.sm, borderWidth: 1, borderColor: colors.primary, borderRadius: radius.sm, backgroundColor: "rgba(44,156,255,.1)" }, deepLinkText: { ...typography.label, color: colors.primary }, deepLinkClear: { minHeight: 44, justifyContent: "center", paddingHorizontal: spacing.sm },
