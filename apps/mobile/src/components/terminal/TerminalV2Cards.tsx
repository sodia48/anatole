...
export function BreadthCard({ snapshot }: { snapshot: TerminalSnapshot }) {
  ...
  return <View style={styles.card} testID="terminal-breadth-pro"><Text style={styles.eyebrow}>BREADTH PRO</Text><View style={styles.grid}>{[
    ...
    metric(pick("Couverture 52S", "52W coverage"), `${data.high_low_52w_eligible_symbols} · ${valueOrNd(data.high_low_52w_coverage_percent, 0, language)} %`),
...
export function RotationCard({ snapshot }: { snapshot: TerminalSnapshot }) {
  ...
  return <View style={styles.card} testID="terminal-rotation-matrix"><Text style={styles.eyebrow}>ROTATION 2.0</Text><Text style={styles.meta}>{pick("Rotation quantitative observée — pas des flux institutionnels", "Observed quantitative rotation — not institutional flows")}</Text><Svg height={height} viewBox={`0 0 ${width} ${height}`} width="100%"><Rect fill="rgba(255,74,104,.05)" height={height / 2} width={width / 2} x="0" y={height / 2} /><Rect fill="rgba(32,202,163,.06)" height={height / 2} width={width / 2} x={width / 2} y="0" /><Line stroke={colors.borderStrong} x1={width / 2} x2={width / 2} y1="0" y2={height} /><Line stroke={colors.borderStrong} x1="0" x2={width} y1={height / 2} y2={height / 2} />{drawable.map((item) => { const cx=scale(item.x!), cy=height-scale(item.y!); const hasPrevious=item.previous_x != null && item.previous_y != null; return <G key={item.sector} onPress={() => router.push({ pathname: "/(tabs)/markets", params: { universe: "tsx60", sector: item.sector } } as Href)} testID={`terminal-rotation-${item.sector}`}>{hasPrevious ? <Line stroke={colors.textMuted} testID={`terminal-rotation-path-${item.sector}`} x1={scale(item.previous_x!)} x2={cx} y1={height-scale(item.previous_y!)} y2={cy} /> : null}<Circle cx={cx} cy={cy} fill={colors.primary} r={Math.max(8, Math.min(19, 6 + item.member_count))} testID={`terminal-rotation-bubble-${item.sector}`} /><SvgText fill="#fff" fontSize="8" textAnchor="middle" x={cx} y={cy - 13}>{item.sector.slice(0, 16)}</SvgText></G>; })}</Svg>{unavailable.map((item) => <Text key={item.sector} style={styles.meta} testID={`terminal-rotation-unavailable-${item.sector}`}>{item.sector} — {pick("données insuffisantes", "insufficient data")} · N/D</Text>)}</View>;
}
