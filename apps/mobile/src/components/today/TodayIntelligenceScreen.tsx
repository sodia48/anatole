...
export function TodayIntelligenceScreen() {
  ...
  const psychology = useQuery({ queryKey: ["psychology"], queryFn: ({ signal }) => marketApi.psychology(signal), enabled: appActive && tier >= 2, staleTime: 120_000 });
  const news = useQuery({ queryKey: ["news", language], queryFn: ({ signal }) => marketApi.news(language, signal), enabled: appActive && tier >= 2, staleTime: 300_000 });
  const calendar = useQuery({ queryKey: ["calendar", language], queryFn: ({ signal }) => marketApi.calendar(language, signal), enabled: appActive && tier >= 2, staleTime: 600_000 });
  const earnings = useQuery({ queryKey: ["earnings", "composite"], queryFn: ({ signal }) => marketApi.earnings(signal), enabled: appActive && tier >= 3, staleTime: 600_000 });
  const watchlist = useQuery({ queryKey: ["watchlist", workspace.data.watchlist], queryFn: ({ signal }) => marketApi.watchlist(workspace.data.watchlist, signal), enabled: appActive && tier >= 3 && workspace.data.watchlist.length > 0, staleTime: 30_000 });
  const alerts = useQuery({ queryKey: ["alerts", workspace.data.alerts], queryFn: ({ signal }) => workspaceApi.alerts(workspace.data.alerts, signal), enabled: appActive && tier >= 3 && workspace.data.alerts.length > 0, staleTime: 30_000 });
  const portfolio = useQuery({ queryKey: ["portfolio", workspace.data.portfolio], queryFn: ({ signal }) => workspaceApi.portfolio(workspace.data.portfolio, signal), enabled: appActive && tier >= 3 && workspace.data.portfolio.length > 0, staleTime: 60_000 });
  const terminal = useQuery<unknown>({ queryKey: ["terminal"], queryFn: ({ signal }) => marketApi.terminal(signal), enabled: appActive && tier >= 4, staleTime: 60_000 });
  const screener = useQuery({ queryKey: ["screener", universe], queryFn: ({ signal }) => marketApi.screener(universe, signal), enabled: appActive && tier >= 4, staleTime: 180_000 });
  const insiders = useQuery({ queryKey: ["insiders", "preview", "canada", 30, ""], queryFn: ({ signal }) => marketApi.insiders({ market: "canada", days: 30, scanLimit: 8 }, signal), enabled: appActive && tier >= 5, staleTime: 300_000 });
  ...
  const renderSection = ({ item }: { item: Section }) => {
    if (item === "header") return <View style={styles.header}><ScreenHeader eyebrow={pick("AUJOURD’HUI 2.0 · DAILY INTELLIGENCE", "TODAY 2.0 · DAILY INTELLIGENCE")} title={`${phase.greeting}${firstName(user?.display_name) ? ` ${firstName(user?.display_name)}` : ""}`} subtitle={phase.title} /><Text style={[styles.marketStatus, phase.quoteIsCurrent && styles.current]}>{phase.marketStatus}</Text></View>;
  ...
  return <SafeAreaView edges={["top"]} style={styles.safe} testID="today-intelligence-screen"><FlatList contentContainerStyle={styles.content} data={sections} initialNumToRender={4} keyExtractor={(item) => item} maxToRenderPerBatch={4} refreshControl={<RefreshControl onRefresh={refresh} refreshing={refreshing} tintColor={colors.primary} />} removeClippedSubviews={false} renderItem={renderSection} windowSize={5} /></SafeAreaView>;
}

const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: colors.background }, content: { gap: spacing.md, padding: spacing.lg, paddingBottom: 120 }, header: { gap: spacing.sm }, marketStatus: { alignSelf: "flex-start", ...typography.caption, color: colors.warning, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs, borderWidth: 1, borderColor: colors.warning, borderRadius: radius.pill }, current: { color: colors.positive, borderColor: colors.positive }, link: { minHeight: 44, justifyContent: "center" }, linkText: { ...typography.caption, color: colors.primary, fontWeight: "800" }, muted: { ...typography.body, color: colors.textMuted }, stale: { ...typography.caption, color: colors.warning }, links: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, nav: { minHeight: 44, minWidth: "46%", flexGrow: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: spacing.sm, borderWidth: 1, borderColor: colors.borderStrong, borderRadius: radius.sm, backgroundColor: colors.surfaceRaised }, navText: { ...typography.label, color: colors.primary },
});
