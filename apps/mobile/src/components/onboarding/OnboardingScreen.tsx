...
export function OnboardingScreen() {
  ...
  const results = useQuery({ queryKey: ["onboarding-search", search], queryFn: ({ signal }) => intelligenceApi.search(search, signal), enabled: step === 3 && search.trim().length >= 2, staleTime: 30 * 60_000 });
  const canContinue = step !== 3 || draft.symbols.length >= 3;
  const progress = `${step + 1}/${ONBOARDING_STEP_COUNT}`;
  ...
  const updateTheme = (next: "dark" | "blue") => { setDraft((current) => ({ ...current, theme: next })); void setTheme(next); };
  ...
  const titles = [
    pick("Votre langue", "Your language"),
    pick("Choisis ton Anatole", "Choose your Anatole"),
    pick("Votre univers", "Your universe"),
    pick("Vos titres", "Your securities"),
    pick("Secteurs d’intérêt", "Sectors of interest"),
    pick("Régions économiques", "Economic regions"),
    pick("Alertes optionnelles", "Optional alerts"),
    pick("Votre Anatole est prêt", "Your Anatole is ready"),
  ];
  ...
  return <Screen testID="onboarding-screen">
    ...
    <ScreenHeader eyebrow="ANATOLE MOBILE" title={titles[step]!} subtitle={step === 3 ? pick("Choisissez 3 à 5 titres. La recherche reste légère.", "Choose 3 to 5 securities. Search remains lightweight.") : undefined} />
...
const styles = createThemedStyles((colors) => ({
  ...
  choice: { minHeight: 44, justifyContent: "center", paddingHorizontal: spacing.md, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceRaised },
  choiceActive: { borderColor: colors.primary, backgroundColor: colors.positiveSurface },
  choiceText: { ...typography.body, color: colors.textMuted },
