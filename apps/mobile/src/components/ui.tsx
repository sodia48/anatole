...
export function Button({ label, onPress, variant = "primary", disabled = false, accessibilityLabel }: { label: string; onPress: () => void; variant?: "primary" | "secondary" | "danger"; disabled?: boolean; accessibilityLabel?: string }) {
  ...
  return (
    ...
    >
      <Text style={[styles.buttonText, variant === "primary" && styles.buttonTextOnColor, variant === "danger" && styles.buttonTextOnColor]}>{label}</Text>
    </Pressable>
  );
}
...
const styles = createThemedStyles((colors) => ({
  ...
  buttonDanger: { backgroundColor: colors.negative, borderWidth: 1, borderColor: colors.negative },
  buttonPressed: { opacity: 0.78 },
  buttonDisabled: { opacity: 0.45 },
  buttonText: { ...typography.label, color: colors.text, fontSize: 14 },
  buttonTextOnColor: { color: colors.onPrimary },
