...
import type { FocusSnapshot, FundamentalSnapshot, StockNewsSnapshot } from "@/src/lib/api/types";
...
export function MobileFocusOverview({
  ticker,
  snapshot,
  period,
  liveState,
  ...
  return <View style={styles.stack} testID="focus-overview-section">
    ...
    <MobileFocusValuation
      error={!fundamentals ? fundamentalsError : null}
      loading={fundamentalsLoading}
      onRetry={onRetryFundamentals}
      snapshot={fundamentals}
    />
    <Card testID="focus-news-section" title={pick("Dernières nouvelles", "Latest news")}><QueryState empty={Boolean(news && news.items.length === 0)} error={!news ? newsError : null} loading={newsLoading} />{news?.items.slice(0, 10).map((item) => <NewsCard item={item} key={item.id} />)}</Card>
    <MobileSessionFlow period={period} snapshot={snapshot} ticker={ticker} />
