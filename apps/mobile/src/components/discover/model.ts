...
export function screenDiscover(rows: ScreenerRow[], strategy: DiscoverStrategy, language: "fr" | "en" = "fr"): DiscoverResult[] {
  ...
  return selected.slice(0, 30).map((row) => ({ ticker: row.ticker, name: row.name, reasons: [strategy === "oversold" ? `RSI ${row.rsi_14?.toFixed(1)}` : strategy === "unusual_volume" ? `Volume relatif ${row.relative_volume.toFixed(1)}x` : strategy === "sector_leaders" ? `${row.sector} · Score ${row.score.toFixed(1)}` : strategy === "breakout" ? (language === "fr" ? `Prix ${(row.breakout_percent ?? 0) >= 0 ? "+" : ""}${(row.breakout_percent ?? 0).toFixed(1)} % au-dessus du plus haut des 20 séances précédentes` : `Price ${(row.breakout_percent ?? 0) >= 0 ? "+" : ""}${(row.breakout_percent ?? 0).toFixed(1)}% above the prior 20-session high`) : `Momentum 20J ${row.momentum_20d >= 0 ? "+" : ""}${row.momentum_20d.toFixed(1)} %`] }));
}
