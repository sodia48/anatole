...
function ResultRow({ item }: { item: ScreenerRow }) {
  ...
  const positive = (value: number) => ({ color: value >= 0 ? colors.positive : colors.negative });
...
export function ScreenerScreen() {
  ...
  const signals = useMemo(() => [...new Set(data?.items.map((item) => item.signal) ?? [])].sort(), [data]);
  ...
  const header = <View style={styles.header}>
    ...
    <View style={styles.summary}><View><Text style={styles.visible}>{rows.length}</Text><Text style={styles.summaryLabel}>{pick("titres visibles", "securities shown")}</Text></View><View style={styles.summaryDetail}><Text style={styles.summaryText}>{data?.live_items ?? 0} live</Text><Text style={styles.summaryText}>{data?.fallback_items ?? 0} fallback</Text>{query.isFetching ? <Text style={styles.refreshing}>{pick("Actualisation…", "Refreshing…")}</Text> : null}</View></View>
