...
type BridgeMessage =
  | { type: "chart-ready" }
  | { type: "point-selected"; buy: number | null; close: number; delta: number | null; high: number; low: number; open: number; sell: number | null; time: string | number; volume: number | null }
...
function selectPoint(event){if(!rows.length)return;const box=layout(),x=event.offsetX;if(x<box.left||x>box.plotRight)return;const step=(box.plotRight-box.left)/rows.length,index=Math.max(0,Math.min(rows.length-1,Math.floor((x-box.left)/step))),row=rows[index],volume=finite(Number(row.volume))&&Number(row.volume)>=0?Number(row.volume):null,buy=volume===null?null:Number(row.close)>Number(row.open)?volume:0,sell=volume===null?null:Number(row.close)<Number(row.open)?volume:0,delta=buy===null||sell===null?null:buy-sell;tip.hidden=false;tip.textContent=formatTime(row.time)+'\\nO '+formatPrice(Number(row.open))+'   H '+formatPrice(Number(row.high))+'\\nL '+formatPrice(Number(row.low))+'   C '+formatPrice(Number(row.close))+'\\nVol '+compactVolume(volume);send({type:'point-selected',buy,close:Number(row.close),delta,high:Number(row.high),low:Number(row.low),open:Number(row.open),sell,time:row.time,volume})}
  ...
  function onMessage(event: WebViewMessageEvent) {
    try {
      ...
      if (message.type === "point-selected") {
        ...
        const flow = message.buy === null || message.sell === null || message.delta === null ? "" : `\n${pick("Achat est.", "Est. buy")} ${compactVolume(message.buy, language)}   ${pick("Vente est.", "Est. sell")} ${compactVolume(message.sell, language)}\n${pick("Delta est.", "Est. delta")} ${message.delta >= 0 ? "+" : ""}${compactVolume(message.delta, language)}`;
        setSelection(`${candleDate(message.time, language)}\nO ${message.open.toFixed(2)}   H ${message.high.toFixed(2)}\nL ${message.low.toFixed(2)}   C ${message.close.toFixed(2)} ${currency}\n${pick("Vol", "Vol")} ${volume}${flow}`);
      }
