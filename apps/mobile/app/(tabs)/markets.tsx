  ...
  return (
    <Screen onRefresh={refresh} refreshing={refreshing} testID="markets-screen">
      ...
      <View style={styles.hubs}>
        {hubs.map((item) => <Pressable accessibilityRole="tab" accessibilityState={{ selected: hub === item.id }} key={item.id} onPress={() => item.id === "psychology" ? router.push("/psychology" as Href) : item.id === "terminal" ? router.push("/terminal" as Href) : item.id === "screener" ? router.push("/screener" as Href) : item.id === "etf" ? router.push("/etf" as Href) : item.id === "ipo" ? router.push("/ipo-insiders" as Href) : setHub(item.id)} style={[styles.hub, hub === item.id && styles.hubActive]}><Text style={[styles.hubText, hub === item.id && styles.hubTextActive]}>{pick(item.fr, item.en)}</Text></Pressable>)}
      </View>
