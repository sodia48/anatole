...
const hubs = [
  ...
  { id: "psychology", fr: "Psychologie", en: "Psychology" },
...
function isHub(value?: string): value is Hub {
  return hubs.some((hub) => hub.id === value);
}
...
function ConstituentsModal({ visible, onClose, items }: { visible: boolean; onClose: () => void; items: MarketTile[] }) {
  ...
  return <Modal animationType="slide" onRequestClose={onClose} visible={visible}><SafeAreaView edges={["top", "bottom"]} style={styles.listSafe}><View style={styles.listHeader}><Text style={styles.listTitle}>{pick("Constituants", "Constituents")}</Text><Pressable accessibilityLabel={pick("Fermer", "Close")} onPress={onClose} style={styles.close}><Text style={styles.closeText}>×</Text></Pressable></View><View style={styles.filters}><Field autoCapitalize="characters" label={pick("Rechercher", "Search")} onChangeText={setSearch} placeholder={pick("RY, SHOP, énergie…", "RY, SHOP, energy…")} value={search} /><FlatList horizontal data={["all", ...sectors]} keyExtractor={(item) => item} renderItem={({ item }) => <Pressable accessibilityState={{ selected: sector === item }} onPress={() => setSector(item)} style={[styles.filter, sector === item && styles.filterActive]}><Text style={styles.filterText}>{item === "all" ? pick("Tous", "All") : item}</Text></Pressable>} showsHorizontalScrollIndicator={false} /></View><FlatList contentContainerStyle={styles.listContent} data={rows} initialNumToRender={14} keyExtractor={(item) => item.ticker} maxToRenderPerBatch={18} removeClippedSubviews renderItem={({ item }) => <StockRow quote={item} />} windowSize={7} /></SafeAreaView></Modal>;
}
...
function MarketsNavigation({ hub, onHub }: { hub: Hub; onHub: (hub: Hub) => void }) {
  ...
  const open = (value: Hub) => {
    ...
    else if (value === "terminal") router.push("/terminal" as Href);
    else if (value === "screener") router.push("/screener" as Href);
    else if (value === "etf") router.push("/etf" as Href);
...
export default function MarketsScreen() {
  ...
  const requestedUniverse = first(params.universe);
  ...
  return <Screen onRefresh={hub === "cockpit" ? () => void cockpit.refetch() : undefined} refreshing={cockpit.isRefetching} testID="markets-screen">
    ...
    {unavailable ? <Card title={hubs.find((item) => item.id === hub)?.fr ?? hub}><Text style={styles.coming}>{pick("Bientôt sur mobile. La migration utilisera le même backend que le web.", "Coming soon to mobile. The migration will use the same backend as the web.")}</Text></Card> : null}
  </Screen>;
}
