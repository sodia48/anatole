    ...
    async def canadian_ticker(
        ...
        *,
        ...
        try:
            ...
            self._ticker_errors.pop(key, None)
        except Exception as exc:
            self._ticker_errors[key] = type(exc).__name__
    ...
    async def snapshot(
        ...
        *,
        ...
        if normalized_market == "Canada":
            ...
            scanned_keys: list[tuple[str, str]] = []
            ...
            failed = sum(
                key in self._ticker_errors
                for key in scanned_keys
            )
            successful = max(0, scanned - failed)
            ...
            sources.extend(
                [
                    InsiderSourceStatus(
                        ...
                        count=len(trades),
                        detail=(
                            f"{successful}/{scanned} titres ont répondu. "
                            "Source secondaire automatisée; "
                            "chaque opération doit être vérifiée dans SEDI."
                        ),
                    ...
                    InsiderSourceStatus(
                        ...
                        detail=(
                            ...
                            "La collecte automatique directe n’est pas "
                            "présentée comme disponible lorsqu’elle ne "
                            "retourne aucun rapport normalisé."
                        ),
                        url=SEDI_URL,
                    ),
                ]
