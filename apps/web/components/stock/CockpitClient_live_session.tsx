...
function latestQuoteTime(
  ...
  const timestamps = snapshot.constituents
    .map((tile) => new Date(tile.timestamp).getTime())
...
export function CockpitClient() {
  const [snapshot, setSnapshot] =
    useState<CockpitSnapshot | null>(null);
    ...
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        if ((reason as Error).name !== "AbortError") {
          setError(
            "Le cockpit n’a pas pu récupérer les données. "
              + "Une nouvelle tentative sera faite automatiquement.",
          );
        }
      } finally {
        setRefreshing(false);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(
      () => void load(),
      REFRESH_INTERVALS.cockpitTsx60,
    );
  ...
  const quoteTime = useMemo(
    () => (snapshot ? latestQuoteTime(snapshot) : "N/D"),
    [snapshot],
  );
    ...
    return (
      <div className="cockpit-page">
        <section className="panel cockpit-loading">
          ...
          <div>
            <h1>Préparation du Cockpit TSX 60</h1>
            <p>
              {error
  ...
  const marketPositive =
    snapshot.weighted_change_percent >= 0;
    ...
    <div className="cockpit-page">
      <header className="panel cockpit-header">
        <div>
          ...
          <p>
            Variation de la séance contre la clôture précédente
            {" · "}
            actualisation environ toutes les 15 secondes
          </p>
        </div>

        <div className="cockpit-market-score">
          <span>Variation de séance</span>
          <strong
            className={
              marketPositive ? "positive" : "negative"
            }
          >
            {marketPositive ? "+" : ""}
          ...
          >
            {refreshing
              ? "Actualisation…"
              : `Dernière cotation ${quoteTime} ET`}
          </small>
        </div>
      </header>

      {error ? (
        <div className="cockpit-warning">{error}</div>
      ) : null}
      ...
      <section className="cockpit-kpis">
        ...
        <article className="panel cockpit-kpi">
          ...
          <strong>
            {snapshot.breadth.advance_ratio.toFixed(0)}%
          </strong>
        </article>
      </section>
      ...
      <section className="cockpit-lower-grid">
        <div className="cockpit-movers-grid">
          <MoversList
            title="Meilleures variations"
            items={snapshot.top_gainers}
          />
          <MoversList
            title="Plus fortes baisses"
            items={snapshot.top_losers}
          />
        </div>
            ...
            {snapshot.sectors.map((sector) => (
              ...
              >
                <div>
                  ...
                  <span>
                    {sector.weight.toFixed(1)}% du panier
                  </span>
                </div>
      ...
      <footer className="status-footer">
        Variation de séance vs clôture précédente
        {" · "}
        Cotations publiques actualisées toutes les 15 s
        et potentiellement différées
        {" · "}
        Univers au {snapshot.universe_as_of}
        {" ("}
        {snapshot.universe_source}
        {")"}
      </footer>
    </div>
  );
