...
from app.schemas.market import CockpitSnapshot, MarketBreadth, MarketTile, SectorSnapshot
...
class CockpitService:
    ...
    async def get_tsx60(self) -> CockpitSnapshot:
        ...
        async with self._lock:
            ...
            weighted_change = sum(tile.weight * tile.change_percent for tile in tiles) / total_weight
            advancers = sum(tile.change_percent > 0.001 for tile in tiles)
            decliners = sum(tile.change_percent < -0.001 for tile in tiles)
            unchanged = len(tiles) - advancers - decliners
            breadth = MarketBreadth(
                advancers=advancers,
                decliners=decliners,
                unchanged=unchanged,
                advance_ratio=round(advancers / max(advancers + decliners, 1) * 100, 2),
            )
            ...
            for sector in sorted({tile.sector for tile in tiles}):
                ...
                sector_change = sum(tile.weight * tile.change_percent for tile in members) / max(sector_weight, 0.0001)
                sectors.append(
                    SectorSnapshot(
                        sector=sector,
                        weight=round(sector_weight, 2),
                        change_percent=round(sector_change, 4),
                        advancers=sum(tile.change_percent > 0.001 for tile in members),
                        decliners=sum(tile.change_percent < -0.001 for tile in members),
                        unchanged=sum(abs(tile.change_percent) <= 0.001 for tile in members),
                    )
                )
            ...
            snapshot = CockpitSnapshot(
                ...
                breadth=breadth,
                sectors=sectors,
                constituents=sorted(tiles, key=lambda tile: tile.weight, reverse=True),
                top_gainers=sorted(tiles, key=lambda tile: tile.change_percent, reverse=True)[:5],
                top_losers=sorted(tiles, key=lambda tile: tile.change_percent)[:5],
